...
it('should check for aws configuration and throw an error if configuration isnt ready', () => {
  jest.doMock('../s3_credentials', () => ({
    default: jest.fn().mockReturnValue({ isConfigReady: false }),
    __esModule: true
  }))

  expect(newS3).toThrowError()
})

describe('when configuration is ready', () => {
  beforeEach(() => {
    jest.doMock('../s3_credentials', () => ({
      __esModule: true,
  ...
  jest.doMock('node-downloader-helper', () => ({
    __esModule: true,
  ...
  it('should create a new AWS', () => {
    newS3()
    const s3cred = require('../s3_credentials').default
  ...
  describe('download', () => {
    it('should use node-downloader-helper and forward errors if any', async () => {
      await expect(newS3().download('example.png', 'f:/usr/tmp')).rejects.toThrowError('Error for f:/usr/tmp')
    ...
    it('should use node-downloader-helper and return filePath when no errors', async () => {
      await expect(newS3().download('meme.png', '/root')).resolves.toEqual('filePath:/root')
  ...
  describe('upload', () => {
    it('should rethrow errors from fs if any', async () => {
      await expect(newS3().upload('x:file')).rejects.toThrowError('x:file')
    })

    it('should rethrow errors from aws-sdk if any', async () => {
      await expect(newS3().upload('file', 'k:key')).rejects.toThrowError('k:key')
    })
